...
\section{\texorpdfstring{$\so(3)$}{so(3)}: Rotation Subalgebra}

\begin{definition}[$\so(3)$]
\label{def:so3}
\lean{FUST.Physics.Lorentz.so3_isomorphism}
The compact part of $\zs$-structure yields the rotation algebra
$\so(3) \cong \R^3$ with $\dim\so(3) = 3$.
\end{definition}
...
\begin{theorem}[$\so(3,1)$ structure]
\label{thm:so31}
\lean{FUST.Physics.Lorentz.so31_isomorphism}
$\so(3,1) \cong \R^6$ with $\dim\so(3,1) = 6$:
3 rotations from $\so(3)$ and 3 boosts from
the non-compact $\vp\ps = -1$ sector.
\end{theorem}
...
The $\Dz$ operator uniquely determines:
\begin{enumerate}
  \item $\re(\afcoeff) = 0$ and $\im(\afcoeff) > 0$
    (1 temporal dimension),
  \item $\phiS$ rank $= 3$
    (3 spatial dimensions),
  \item $\phiA[\mathrm{id}](1) \ne 0$
    (temporal channel nontrivial),
  \item Signature $(1,3)$: temporal direction gives positive norm squared,
    spatial directions give negative norm squared,
