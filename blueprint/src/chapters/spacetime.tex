...
and the signature $(1,3)$ is derived from
the Galois norm structure of $\Q(\sqrt{5},\sqrt{-3})$:
$\vp\ps = -1$ (indefinite/timelike) vs.\ $\zs\zsc = +1$ (definite/spacelike).

\section{Metric Signature from Galois Norms}

The two Galois conjugations of $\Q(\sqrt{5},\sqrt{-3})$ determine the metric:
\begin{itemize}
  \item $\sigma_1$: $\vp \leftrightarrow \ps$, with $\vp\ps = -1$
    $\Rightarrow$ AF channel norm is \emph{negative} $\Rightarrow$ timelike,
  \item $\sigma_2$: $\zs \leftrightarrow \zsc$, with $\zs\zsc = +1$
    $\Rightarrow$ SY channel norm is \emph{positive} $\Rightarrow$ spacelike.
\end{itemize}

\begin{theorem}[AF sign from $\vp\ps$]
\label{thm:AF-sign}
\lean{FUST.Physics.Lorentz.AF_sign_from_phi_psi}
$\vp\ps = -1$.
\end{theorem}

\begin{theorem}[SY sign from $\zs\zsc$]
\label{thm:SY-sign}
\lean{FUST.Physics.Lorentz.SY_sign_from_zeta6}
$\zs\zsc = 1$.
\end{theorem}

\begin{theorem}[$\afcoeff^2 < 0$]
\label{thm:AF-sq-neg}
\lean{FUST.Physics.Lorentz.AF_coeff_sq_negative}
$\re(\afcoeff^2) < 0$, since $\afcoeff = 2i\sqrt{3}$ is pure imaginary.
\end{theorem}

\begin{theorem}[Signature from $\Dz$]
\label{thm:signature-Dz}
\lean{FUST.Physics.Lorentz.signature_from_Dζ}
Combining the above:
$\vp\ps = -1$, $\zs\zsc = 1$, $\re(\afcoeff^2) < 0$, $6^2 > 0$,
and $|{\mathrm{Fin}\,1}| + |{\mathrm{Fin}\,3}| = 4$.
\end{theorem}
...
$I_4 = \mathrm{Fin}\,1 \oplus \mathrm{Fin}\,3$: spacetime index from $\Dz$ channels
(1 AF + 3 SY sub-operators).
\end{definition}
...
\begin{enumerate}
  ...
  \item Signature $(1,3)$: $(6a)^2 > 0$ (spacelike, SY), $(\afcoeff \cdot b)^2 < 0$ (timelike, AF),
