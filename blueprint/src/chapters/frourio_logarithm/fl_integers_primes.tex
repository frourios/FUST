...
\label{def:phi-orbit}
\lean{FUST.FrourioLogarithm.phiOrbit}
\leanok
For $x_0 > 0$, the $\varphi$-orbit is:
\[
\mathrm{Orbit}_\varphi(x_0) := \{ y \mid \exists k \in \mathbb{Z},\, y = x_0 \cdot \varphi^k \}
\]
\end{definition}

\begin{definition}[Orbit Label]
\label{def:orbit-label}
\lean{FUST.FrourioLogarithm.OrbitLabel}
\leanok
An orbit label consists of:
\begin{itemize}
    \item A base point $x_0 > 0$
    \item An integer index $k \in \mathbb{Z}$
\end{itemize}
\end{definition}

\begin{theorem}[Orbit Label Additivity]
\label{thm:orbit-label-additive}
\lean{FUST.FrourioLogarithm.orbit_label_additive}
\leanok
The Frourio coordinate of an orbit label is:
\[
t(x_0, k) = \log_{\mathfrak{f}}(x_0) + k \cdot \mathrm{phiStep}
\]
This is additive in the index $k$.
\end{theorem}

The key insight: integers $k$ index positions along the $\varphi$-orbit, forming a $\mathbb{Z}$-torsor in Frourio space.
