...
\leanok
\[
z_P = \frac{\ell_P}{\varphi^3}, \qquad \ell_P = \texttt{structuralMinTimeD6} = \frac{25}{12}
\]
\end{definition}
...
\leanok
\[
0 < z < z_P \Rightarrow \varphi^3 z < \ell_P
\]
\end{theorem}
...
\leanok
\[
z \geq z_P \Rightarrow \varphi^3 z \geq \ell_P
\]
\end{theorem}
...
\begin{enumerate}
\item $\ell_P > 0$ (structural minimum length from $D_\zeta$ is positive)
\item Third law: $f \notin \ker(D_6) \Rightarrow \exists t,\; S(f,t) > 0$ (massive states have positive entropy)
\item $\forall n \geq 3: C_n^2 > 0$ (dissipation is active outside $\ker(D_6)$, the spatial DOF)
\end{enumerate}
\end{theorem}
...
The 3 spatial kernel modes ($n \leq 2$, from $\dim\ker(D_6) = 3$) remain stationary: $C_n = 0$ means no dissipation acts on them.
