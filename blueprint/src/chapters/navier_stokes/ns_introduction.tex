...
The AF-channel projection $N_6$ (antisymmetric, $r = 1$) provides the dissipation structure on the $\varphi$-lattice. FUST replaces the continuous PDE with an algebraic mode evolution system:
\begin{equation}\label{eq:ns_mode_ode}
\frac{d\hat{u}_n}{dt} = -C_n^2 \hat{u}_n + \sum_{m+k=n} N_{m,k}\, \hat{u}_m \hat{u}_k
\end{equation}
where $C_n$ is the spectral coefficient of $D_\zeta$'s AF-channel projection $N_6$, and $N_{m,k}$ is the Leibniz deviation.
...
\begin{enumerate}
\item \textbf{$D_\zeta$ AF-channel resolution}: $N_6$ evaluates at $\{\varphi^3 z, \varphi^2 z, \varphi z, \psi z, \psi^2 z, \psi^3 z\}$. When $z$ is too small, sampling points fall below $\ell_P = 25/12$
...
\begin{itemize}
\item $D_\zeta$'s AF-channel $N_6$ acts as a 6-point sampling operator on the $\varphi^n$-lattice
