...
The AF-channel projection $D_6$ (antisymmetric, $r = 1$) provides the dissipation structure on the $\varphi$-lattice. FUST replaces the continuous PDE with an algebraic mode evolution system:
\begin{equation}\label{eq:ns_mode_ode}
\frac{d\hat{u}_n}{dt} = -C_n^2 \hat{u}_n + \sum_{m+k=n} N_{m,k}\, \hat{u}_m \hat{u}_k
\end{equation}
where $C_n$ is the spectral coefficient of $D_\zeta$'s AF-channel projection $D_6$, and $N_{m,k}$ is the Leibniz deviation.

\subsection{Key Structural Properties from \texorpdfstring{$D_\zeta$}{Dζ}}
\begin{enumerate}
\item \textbf{4D spacetime}: $D_\zeta$ determines $\dim\ker(D_6) + 1 = 3 + 1 = 4$ via $I_4 = \mathrm{Fin}\,3 \oplus \mathrm{Fin}\,1$
\item \textbf{Spatial DOF}: $\ker(D_6) = \mathrm{span}\{1, z, z^2\}$ (dim 3 = spatial dimensions)
...
\item \textbf{Kernel stability}: $\ker(D_6)$ is time-invariant (spatial steady state)
...
\begin{enumerate}
\item \textbf{$D_\zeta$ AF-channel resolution}: $D_6$ evaluates at $\{\varphi^3 z, \varphi^2 z, \varphi z, \psi z, \psi^2 z, \psi^3 z\}$. When $z$ is too small, sampling points fall below $\ell_P = 25/12$
...
\begin{itemize}
\item $D_\zeta$'s AF-channel $D_6$ acts as a 6-point sampling operator on the $\varphi^n$-lattice
...
\item Sub-Planck modes ($n > N_{\max}$): set to $0$ (below $D_\zeta$ resolution)
\end{itemize}
