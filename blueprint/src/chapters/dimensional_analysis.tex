...
The dissipation coefficient $C_d$ is the value of $\mathrm{Diff}_6$ applied to $x^d$.
Even-$m$ operators ($\mathrm{Diff}_2, \mathrm{Diff}_4, \mathrm{Diff}_6$) have antisymmetric coefficient patterns yielding $\sqrt{5} \cdot F_n$ (Fibonacci) factors,
giving $\delta = 1$.
Odd-$m$ operators ($\mathrm{Diff}_3, \mathrm{Diff}_5$) have symmetric patterns yielding Lucas integers, giving $\delta = 0$.
...
\hline
$\mathrm{Diff}_2$ & antisymmetric (Fibonacci) & 1 & $(-1, 1)$ \\
$\mathrm{Diff}_3$ & symmetric (Lucas) & 0 & $(-2, 0)$ \\
$\mathrm{Diff}_4$ & antisymmetric (Fibonacci) & 1 & $(-3, 1)$ \\
$\mathrm{Diff}_5$ & symmetric (Lucas) & 0 & $(-4, 0)$ \\
$\mathrm{Diff}_6$ & antisymmetric (Fibonacci) & 1 & $(-5, 1)$ \\
\hline
\end{tabular}
\end{center}
...
\begin{definition}[Per-Operator Time Dimension: $\mathrm{Diff}_2$]
\label{def:dim_time_D2}
\lean{FUST.Dim.dimScale}
\leanok
\[
\mathrm{dimScale} := (\dim(\mathrm{Diff}_2 f))^{-1} = (1, -1)
\]
...
$\Phi_S f(x) = \mathrm{Diff}_5 f(x) + \frac{2}{\varphi} \cdot x \cdot \mathrm{Diff}_2 f(x)$ has a direct sum type:
\[
\Phi_S f(x) \in \mathrm{ScaleQ}(-4, 0) \oplus \mathrm{ScaleQ}(-1, 1)
\]
The $\mathrm{Diff}_5$ component has dimension $(-4, 0)$ and the $\mathrm{Diff}_2$ component has dimension $(-1, 1)$.
...
Each particle has a unique FDim $= \mathrm{deriveFDim}(6)^a \times \mathrm{dimScale}^n$, where
...
$\mathrm{Diff}_6 f(x)$ & $(-5, 1)$ & $\mathrm{Diff}_6$ & \texttt{D6\_dim} \\
$(\mathrm{Diff}_6 f)^2$ & $(-10, 2)$ & $F_\zeta^2$ & \verb|dimSector^2| \\
$\Delta = m_e$ & $(-5, 1)$ & $\mathrm{Diff}_6$ & \texttt{dimElectron} \\
$m_\mu = \Delta \cdot \varphi^{11}$ & $(6, -10)$ & $\mathrm{Diff}_6$ & \texttt{dimMuon} \\
$m_\tau = \Delta \cdot \varphi^{17}$ & $(12, -16)$ & $\mathrm{Diff}_6$ & \texttt{dimTau} \\
$m_p$ & $(9, -13)$ & $\mathrm{Diff}_6$ & \texttt{dimProton} \\
$m_n$ & $(8, -12)$ & $\mathrm{Diff}_6 \times \mathrm{Diff}_2$ & \texttt{dimNeutron} \\
$m_W$ & $(20, -24)$ & $\mathrm{Diff}_6$ & \texttt{dimWBoson} \\
$m_{\nu_3}$ & $(-42, 34)$ & $F_\zeta^2$ & \texttt{dimNu3} \\
$m_{\nu_2}$ & $(-43, 35)$ & $F_\zeta^2 \times \mathrm{Diff}_2$ & \texttt{dimNu2} \\
\hline
$m_Z^2$ (comp1) & $(40, -48)$ & $F_\zeta^2$ & \texttt{dimZSqComp1} \\
$m_Z^2$ (comp2) & $(42, -46)$ & Mixed & \texttt{dimZSqComp2} \\
$m_H$ (comp1) & $(21, -25)$ & $\mathrm{Diff}_6$ & \texttt{dimHiggsVacuum} \\
$m_H$ (comp2) & $(18, -22)$ & $\mathrm{Diff}_6$ & \texttt{dimHiggsCorrection} \\
\hline
...
\hline
$\mathrm{card}(\mathrm{Fin}\,2) = \dim\ker(\mathrm{Diff}_5)$ & 2 & \texttt{Fintype.card (Fin 2)} \\
...
$(\mathrm{Diff}_6 f)^2$ Hamiltonian & Scale$(-10,2)$ $\checkmark$ \\
$\mathrm{Diff}_6[\mathrm{timeEvolution}(f)] = \varphi \cdot \mathrm{Diff}_6[f](\varphi \cdot)$ & Both sides: $(-5,1)$ $\checkmark$ \\
\hline
\end{tabular}
\end{center}
\end{theorem}
...
Time evolution amplifies $\mathrm{Diff}_6$ by $\varphi$:
\[
\mathrm{Diff}_6[\mathrm{timeEvolution}(f)](x) = \varphi \cdot \mathrm{Diff}_6[f](\varphi x)
\]
...
$(\mathrm{Diff}_6 f)^2 : (-10, 2)$ & Metric $g_{00}$ \\
...
Every FDim decomposes as $\mathrm{deriveFDim}(6)^a \times \mathrm{dimScale}^n$ with $(a,n) \in \mathbb{Z}^2$:
\begin{itemize}
...
\item \textbf{detectionLevel} $n = d - 3a$: $\mathrm{Diff}_2$ iterations beyond $F_\zeta^a$ baseline
\end{itemize}

\section{Degree Constraint from Mass Condition}
\label{sec:dim_degree_constraint}

The BH formation condition $|\mathrm{Diff}_6(x^d)(x_0)| < 1$ constrains admissible polynomial degrees.
For monomial state functions $g(x) = x^d$:
\[
\mathrm{Diff}_6(x^d)(x_0) = C_d \cdot x_0^{d-1}, \quad C_d = \frac{F_{3d} - 3F_{2d} + F_d}{25}
\]
...
\begin{theorem}[\texorpdfstring{$\mathrm{Diff}_6(x^4)$}{Diff6(x\^{}4)} Quartic Formula]
\label{thm:dim_d6_quartic}
\lean{FUST.Diff6_quartic_eq}
\leanok
\[
\mathrm{Diff}_6(x^4)(x_0) = \frac{84}{25} \cdot x_0^3
\]
...
\lean{FUST.Diff6_cubic_admissible_in_domain}
\leanok
For $x_0 \in (0, 1]$ (FUST coordinate domain):
\[
|\mathrm{Diff}_6(x^3)(x_0)| = \frac{12}{25} \cdot x_0^2 \leq \frac{12}{25} < 1
\]
...
\lean{FUST.Diff6_quartic_inadmissible_at_one}
\leanok
\[
\mathrm{Diff}_6(x^4)(1) = \frac{84}{25} = 3.36 > 1
\]
...
\leanok
A mode $(d, x_0)$ is \emph{admissible} if $x_0 \neq 0 \Rightarrow |\mathrm{Diff}_6(x^d)(x_0)| < 1$.
\end{definition}
...
The critical threshold $k = 3$ corresponds to the highest node degree $\varphi^3$ in $\mathrm{Diff}_6$.
