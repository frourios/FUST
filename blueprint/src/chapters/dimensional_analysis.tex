...
In FUST, quantities such as $t_{\mathrm{FUST}} = 25/12$ and $\Delta = 12/25$ are defined as $\mathbb{R}$-valued (real part of $\mathbb{C}$) in the Lean implementation.
This allows type-checked operations like $t_{\mathrm{FUST}} + \Delta$ that are physically meaningless.
A time and a mass gap may share the same numerical value yet represent fundamentally different physical quantities.
...
\leanok
\[
\dim(D_6 f) = (-5, 1) = \dim(t_{\mathrm{FUST}})^{-1}
\]
\end{theorem}
...
\leanok
\[
\dim(t_{\mathrm{FUST}}) := (\dim(D_6 f))^{-1} = (5, -1)
\]
\end{definition}
...
\hline
$t_{\mathrm{FUST}} = 25/12$ & $(5, -1)$ & --- & \texttt{structuralMinTime\_dim} \\
...
\hline
$t_{\mathrm{FUST}} = 25/12$ & Scale$(5,-1)$ $\checkmark$ \\
$\Delta = 12/25 = 1/t_{\mathrm{FUST}}$ & Scale$(-5,1)$ $\checkmark$ \\
