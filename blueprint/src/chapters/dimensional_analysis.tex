...
In FUST, quantities such as $t_{\mathrm{FUST}} = 25/12$ and $\Delta = 6$ are all defined as $\mathbb{R}$ or $\mathbb{N}$ in the Lean implementation.
...
This chapter introduces a \textbf{structural type system} $(p, \delta, \tau)$ derived from FUST's internal algebraic structure,
...
\lean{FUST.Dim.FDim}
\leanok
The dimension of a FUST ScaleQuantity is a triple $(p, \delta, \tau) \in \mathbb{Z}^3$:
\begin{itemize}
\item $p$: \textbf{$\varphi$-power dimension} --- exponent of $(\varphi - \psi) = \sqrt{5}$
...
\subsection{Dimension I: \texorpdfstring{$\varphi$}{φ}-Power Dimension \texorpdfstring{$p$}{p}}
...
\leanok
$\mathrm{ScaleQ}(p, \delta, \tau)$: a real-valued quantity carrying dimension $(p, \delta, \tau)$.
...
Carries no $(p, \delta, \tau)$ structure.
\end{definition}
...
\leanok
$(p, \delta, \tau)$ forms a commutative group under componentwise addition:
\begin{align}
\dim(Q_1 \cdot Q_2) &= (p_1 + p_2,\; \delta_1 + \delta_2,\; \tau_1 + \tau_2) \\
\dim(Q_1 / Q_2) &= (p_1 - p_2,\; \delta_1 - \delta_2,\; \tau_1 - \tau_2)
\end{align}
\end{theorem}
...
\hline
Operator & Symmetry & $\delta$ & $\tau$ & $(p, \delta, \tau)$ \\
\hline
...
\]
Length shares $(p, \delta) = (5, -1)$ with time but has $\tau = 0$ (spatial).
\end{definition}
...
\begin{tabular}{|c|c|c|}
\hline
Quantity & $(p, \delta, \tau)$ & Lean reference \\
\hline
...
Mass gap $\Delta$ & 6 & \texttt{massGap} \\
...
\lean{FUST.Dim.spectralGap_count}
\leanok
$\Delta = 6$ is CountQuantity, $E = (D_6 f)^2$ is ScaleQuantity$(-10, 2, -2)$.
The comparison $\Delta^2 \leq E$ in \texttt{EnergyInSpectrum} is a numerical comparison in the FUST coordinate system,
not an equality of dimensioned quantities.
\end{theorem}

The distinction between $\Delta = 6$ (CountQuantity from $\dim\ker(D_5) \times \dim\ker(D_6)$)
and $\varphi^6 \approx 17.94$ (ScaleQuantity, mass ratio $m_s/m_d$) is made explicit by the type system,
despite the coincidental shared exponent~6.
