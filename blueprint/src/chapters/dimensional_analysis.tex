...
A time and a length may share the same numerical value yet represent fundamentally different physical quantities.
...
The time dimension $\tau$ measures how deeply an operator probes beyond the spatial kernel.
Specifically, $\tau_m = 2 - \mathrm{operatorKerDim}(m)$ where $\mathrm{operatorKerDim}(m)$
is the number of standard monomial basis elements annihilated by $D_m$:
\begin{itemize}
\item $D_2, D_3, D_4$: $\mathrm{kerDim} = 1$, hence $\tau = 2 - 1 = 1$
\item $D_5$: $\mathrm{kerDim} = 2$, hence $\tau = 2 - 2 = 0$
\item $D_6$: $\mathrm{kerDim} = 3$, hence $\tau = 2 - 3 = -1$
\end{itemize}
Positive $\tau$ means the operator detects modes below $\ker(D_6)$ (sub-spatial),
$\tau = 0$ means it detects exactly the boundary, and $\tau = -1$ means it detects beyond (temporal).
...
\]
where $\delta_m = [m \bmod 2 = 0]$ and $\tau_m = 2 - \mathrm{operatorKerDim}(m)$.
\end{definition}

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
Operator & Symmetry & $\delta$ & kerDim & $\tau$ & $(\sqrt{5}, \delta, \tau)$ \\
\hline
$D_2$ & antisymmetric (Fibonacci) & 1 & 1 & 1 & $(-1, 1, 1)$ \\
$D_3$ & symmetric (Lucas) & 0 & 1 & 1 & $(-2, 0, 1)$ \\
$D_4$ & antisymmetric (Fibonacci) & 1 & 1 & 1 & $(-3, 1, 1)$ \\
$D_5$ & symmetric (Lucas) & 0 & 2 & 0 & $(-4, 0, 0)$ \\
$D_6$ & antisymmetric (Fibonacci) & 1 & 3 & $-1$ & $(-5, 1, -1)$ \\
\hline
\end{tabular}
\end{center}
...
\lean{FUST.Dim.dimLength_eq_speed_times_time}
\leanok
\[
...
\subsection{Per-Operator Time Dimensions}

Each $D_m$ defines its own time dimension $\mathrm{dimTime}_{D_m} = (\dim(D_m f))^{-1}$.
These are all distinct, reflecting the distinct roles of each operator.

\begin{definition}[Per-Operator Time Dimensions]
\label{def:dim_time_per_operator}
\lean{FUST.Dim.dimTimeD2}
\lean{FUST.Dim.dimTimeD3}
\lean{FUST.Dim.dimTimeD4}
\lean{FUST.Dim.dimTimeD5}
\leanok
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Operator & $\mathrm{dimTime}_{D_m}$ & Interpretation \\
\hline
$D_2$ & $(1, -1, -1)$ & Fibonacci scale, dissipative, temporal \\
$D_3$ & $(2, 0, -1)$ & Lucas scale, non-dissipative, temporal \\
$D_4$ & $(3, -1, -1)$ & Fibonacci scale, dissipative, temporal \\
$D_5$ & $(4, 0, 0)$ & Lucas scale, non-dissipative, spatial \\
$D_6$ & $(5, -1, 1)$ & Fibonacci scale, dissipative, temporal \\
\hline
\end{tabular}
\end{center}
\end{definition}

\begin{theorem}[Per-Operator Dimensional Distinctness]
\label{thm:dim_per_operator_distinct}
\lean{FUST.Dim.dimTime_all_distinct}
\leanok
All five per-operator time dimensions are pairwise distinct:
\[
\mathrm{dimTime}_{D_i} \neq \mathrm{dimTime}_{D_j} \quad \text{for } i \neq j.
\]
\end{theorem}

\begin{theorem}[$D_5$ Spatial Anomaly]
\label{thm:dim_D5_spatial}
\lean{FUST.Dim.dimTimeD5_tau}
\lean{FUST.Dim.dimTimeD5_eq_dimLengthD5}
\leanok
$D_5$ is the unique operator with $\tau = 0$:
\[
\mathrm{dimTime}_{D_5} = (4, 0, 0) = \mathrm{dimLength}_{D_5}.
\]
Time and length coincide for $D_5$, reflecting that $D_5$ cannot distinguish temporal from spatial modes
(its kernel $\{1, x\}$ lies entirely within the spatial sector of $\ker(D_6)$).
\end{theorem}

Each operator also defines its own length and light speed:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
Operator & $\mathrm{dimTime}_{D_m}$ & $\mathrm{dimLength}_{D_m}$ & $\mathrm{dimLightSpeed}_{D_m}$ \\
\hline
$D_2$ & $(1, -1, -1)$ & $(1, -1, 0)$ & $(0, 0, 1)$ \\
$D_3$ & $(2, 0, -1)$ & $(2, 0, 0)$ & $(0, 0, 1)$ \\
$D_4$ & $(3, -1, -1)$ & $(3, -1, 0)$ & $(0, 0, 1)$ \\
$D_5$ & $(4, 0, 0)$ & $(4, 0, 0)$ & $(0, 0, 0)$ \\
$D_6$ & $(5, -1, 1)$ & $(5, -1, 0)$ & $(0, 0, -1)$ \\
\hline
\end{tabular}
\end{center}

$D_5$ has $\mathrm{dimLightSpeed}_{D_5} = (0,0,0)$, meaning speed is dimensionless in the $D_5$ sector.
$D_6$ gives $\mathrm{dimLightSpeed} = (0,0,-1)$, the physical light speed dimension.
...
\[
D_{5\frac{1}{2}} f(x) \in \mathrm{ScaleQ}(-4, 0, 0) \oplus \mathrm{ScaleQ}(-1, 1, 1)
\]
The $D_5$ component has dimension $(-4, 0, 0)$ and the $D_2$ component has dimension $(-1, 1, 1)$.
...
\begin{tabular}{|c|c|c|c|}
\hline
Quantity & $(\sqrt{5}, \delta, \tau)$ & Sector & Lean reference \\
\hline
$t_{\mathrm{FUST}} = 25/12$ & $(5, -1, 1)$ & --- & \texttt{structuralMinTime\_dim} \\
$c = 1$ & $(0, 0, -1)$ & --- & \texttt{dimLightSpeed} \\
$D_6 f(x)$ & $(-5, 1, -1)$ & $D_6$ & \texttt{D6\_dim} \\
$(D_6 f)^2$ & $(-10, 2, -2)$ & $D_6^2$ & \texttt{dimLagrangian} \\
$\Delta = m_e$ & $(-5, 1, -1)$ & $D_6$ & \texttt{dimElectron} \\
$m_\mu = \Delta \cdot \varphi^{11}$ & $(6, -10, -12)$ & $D_6$ & \texttt{dimMuon} \\
$m_\tau = \Delta \cdot \varphi^{17}$ & $(12, -16, -18)$ & $D_6$ & \texttt{dimTau} \\
$m_p$ & $(9, -13, -15)$ & $D_6$ & \texttt{dimProton} \\
$m_n$ & $(8, -12, -14)$ & $D_6 \times D_2$ & \texttt{dimNeutron} \\
$m_W$ & $(20, -24, -26)$ & $D_6$ & \texttt{dimWBoson} \\
$m_{\nu_3}$ & $(-42, 34, 30)$ & $D_6^2$ & \texttt{dimNu3} \\
$m_{\nu_2}$ & $(-43, 35, 31)$ & $D_6^2 \times D_2$ & \texttt{dimNu2} \\
\hline
$m_Z^2$ (comp1) & $(40, -48, -52)$ & $D_6^2$ & \texttt{dimZSqComp1} \\
$m_Z^2$ (comp2) & $(42, -46, -52)$ & Mixed & \texttt{dimZSqComp2} \\
$m_H$ (comp1) & $(21, -25, -27)$ & $D_6$ & \texttt{dimHiggsVacuum} \\
$m_H$ (comp2) & $(18, -22, -24)$ & $D_6$ & \texttt{dimHiggsCorrection} \\
\hline
$m_s/m_d = \varphi^6$ & $(0, 0, 0)$ & Ratio & \texttt{msMdRatio} \\
$\alpha = \varphi^{-5}/4\pi$ & $(0, 0, 0)$ & Ratio & \texttt{fineStructure} \\
\hline
\end{tabular}
\end{center}
