...
\begin{tabular}{|c|c|c|}
\hline
Gauge boson & Symmetry origin & Mass mechanism \\
\hline
Gluon (8) & $\ker(D_6)$, $\mathrm{SU}(3)$ & In $\ker(D_6)$: $D_6(\text{gluon}) = 0$ $\to$ massless \\
Photon & Polynomial grading, $\mathrm{U}(1)$ & Grading preserved by all $D_m$ $\to$ massless \\
$W^\pm, Z$ & $\ker(D_5)$, $\mathrm{SU}(2)$ & $D_5(x^2) \neq 0$: couples to Higgs DOF $\to$ massive \\
...
\]
with 0.002\% agreement against PDG data. See Chapter~\ref{chap:pd_mass_ratios} for details.
