...
\lean{FUST.QuarkMassRatios.mu_md_from_Diff2}
\leanok
\[
\frac{m_u}{m_d} = \frac{1}{2}
\]
from $\mathrm{Diff}_2$ isospin structure. Experimental: 0.47 (error 6.4\%).
\end{theorem}
...
\]
where $C(4,2) = 6$ is the $\mathrm{Diff}_4$ pair count. Experimental: 19.5 (error $\sim 8\%$).
\end{theorem}
...
$\mu$ (middle) & 11 & $C(5,2)+C(2,2)$ & $\mathrm{Diff}_5 + \mathrm{Diff}_2$ pairs \\
$\tau$ (heaviest) & 17 & $C(4,2)+C(5,2)+C(2,2)$ & $\mathrm{Diff}_4 + \mathrm{Diff}_5 + \mathrm{Diff}_2$ pairs \\
\hline
\end{tabular}
\end{center}
...
\]
The exponent $6 = C(4,2)$ is the $\mathrm{Diff}_4$ pair count.
\end{theorem}
...
\]
The exponent $11 = C(5,2) + C(2,2) = 10 + 1$ combines $\mathrm{Diff}_5$ and $\mathrm{Diff}_2$ pair counts.
\end{theorem}
...
\begin{theorem}[$\mathrm{Diff}_6$ Correction Factor]
\label{thm:mass_diff6_correction}
\lean{FUST.MassRatioDerivation.Diff6_correction_6pt}
\leanok
\[
...
\begin{theorem}[$\mathrm{Diff}_5$ Correction Factor]
\label{thm:mass_diff5_correction}
\lean{FUST.MassRatioDerivation.Diff5_correction_11pt}
\leanok
\[
...
Each particle has a unique FDim $= \mathrm{deriveFDim}(6)^a \times \mathrm{dimScale}^n$.
...
\]
The electron, as the lightest \emph{charged} fermion, is identified with the $F_\zeta$ minimum eigenvalue. Neutrinos acquire mass via $\ker(\mathrm{Diff}_5) \subset \ker(F_\zeta)$ perturbation (see \S\ref{sec:mass_neutrino}) and are not bounded by $\lambda_{\min}$.
\end{theorem}
...
\item $C(4,2) = 6$: baryon pair count from $\mathrm{Diff}_4$ hierarchy
\item $C(3,2)+C(5,2) = 13$: kernel and $\mathrm{Diff}_5$ pair normalization
\end{itemize}
...
The W boson mass is derived from the kernel hierarchy $\ker(\mathrm{Diff}_5) \subsetneq \ker(F_\zeta)$.
The key degree of freedom is $x^2 \in \ker(F_\zeta) \setminus \ker(\mathrm{Diff}_5)$: this acts as the Higgs mechanism,
...
\[
\mathrm{dimDarkMatter} = \mathrm{deriveFDim}(\Phi_S) \times \mathrm{dimScale}^{25} = (21, -25, -24)
...
but their mass structure is governed by $\ker(\mathrm{Diff}_5) \subset \ker(F_\zeta)$:

\begin{itemize}
\item $\ker(\mathrm{Diff}_5) = \{1, x\}$ ($\dim = 2$): solar pair $\nu_1, \nu_2$ (nearly degenerate)
\item $\ker(F_\zeta) \setminus \ker(\mathrm{Diff}_5) = \{x^2\}$ ($\dim = 1$): atmospheric state $\nu_3$ (separated)
\item $\mathrm{Diff}_5(x^2) \neq 0$: $x^2$ is not in $\ker(\mathrm{Diff}_5)$, giving $\nu_3$ its distinct mass
\end{itemize}
...
\leanok
\[
\frac{\Delta m^2_{21}}{\Delta m^2_{31}} = \frac{1}{\dim\ker(\mathrm{Diff}_5) \times C(6,2)} = \frac{1}{2 \times 15} = \frac{1}{30}
\]
where $\dim\ker(\mathrm{Diff}_5) = 2$ counts the solar pair states and $C(6,2) = 15$ is the $F_\zeta$ pair count.
...
\leanok
The kernel filtration $\ker(\mathrm{Diff}_2) \subset \ker(\mathrm{Diff}_5) \subset \ker(F_\zeta)$ with $\dim = 1, 2, 3$
determines the neutrino mass hierarchy:
\begin{enumerate}
\item $\dim\ker(\mathrm{Diff}_5) = 2$: two nearly-degenerate states (solar pair)
\item $\mathrm{Diff}_5(x^2) \neq 0$: one separated state (atmospheric)
\item $\text{neutrinoMassSqDenom} = \dim\ker(\mathrm{Diff}_5) \times C(6,2) = 30$
\end{enumerate}
\end{theorem}

Neutrino masses are not bounded by $\lambda_{\min}$: neutrinos acquire mass via $\mathrm{Diff}_5$-level perturbation from $\ker(F_\zeta)$. The $F_\zeta$ minimum eigenvalue $\lambda_{\min}$ applies only to charged fermions that couple directly to $F_\zeta$ output.
...
$\Delta m^2_{21}/\Delta m^2_{31} = 1/(\dim\ker(\mathrm{Diff}_5) \times C(6,2)) = 1/30$ & RatioQuantity & \texttt{neutrinoMassSqRatio} \\
\hline
\end{tabular}
\end{center}
...
$m_s/m_d = \varphi^6$ (ScaleQuantity, $\approx 17.94$) is structurally distinct from $\lambda_{\min} = 12/25$ (ScaleQuantity at dimSector, $F_\zeta$ minimum eigenvalue from gauge-invariant output).
