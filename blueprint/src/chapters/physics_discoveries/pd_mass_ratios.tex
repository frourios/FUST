...
\section{Lepton Mass Ratios from $\ker(D_6)$ Basis}
\label{sec:mass_lepton}

The three lepton generations correspond to $\ker(D_6) = \{1, x, x^2\}$.
Each basis element $t^n$ maps to a $D$-operator index $k$, with mass exponent $T(k) + \delta$:
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
$\ker(D_6)$ basis & Generation & $D$-index & Exponent & Reason \\
\hline
...
\]
The electron, as the lightest \emph{charged} fermion, is identified with the mass gap. Neutrinos acquire mass via $\ker(D_5) \subset \ker(D_6)$ perturbation (see \S\ref{sec:mass_neutrino}) and are not bounded by $\Delta$.
\end{theorem}
...
Neutrino masses are not bounded by $\Delta$: neutrinos acquire mass via $D_5$-level perturbation from $\ker(D_6)$. The mass gap $\Delta$ applies only to charged fermions that couple directly to $D_6$ output.
