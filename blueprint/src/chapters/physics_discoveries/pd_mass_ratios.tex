...
\]
where $C(4,2) = 6$ is the $D_4$ pair count. Experimental: 19.5 (error $\sim 8\%$).
\end{theorem}
...
Each lepton mass exponent is a sum of D-operator pair counts $C(m,2)$:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
Lepton & Exponent & Pair count sum & Reason \\
\hline
$e$ (lightest) & 0 & base & $D_6$ mass gap \\
$\mu$ (middle) & 11 & $C(5,2)+C(2,2)$ & $D_5 + D_2$ pairs \\
$\tau$ (heaviest) & 17 & $C(4,2)+C(5,2)+C(2,2)$ & $D_4 + D_5 + D_2$ pairs \\
\hline
\end{tabular}
\end{center}
...
\]
The exponent $6 = C(4,2)$ is the $D_4$ pair count.
\end{theorem}
...
\]
The exponent $11 = C(5,2) + C(2,2) = 10 + 1$ combines $D_5$ and $D_2$ pair counts.
\end{theorem}
...
\]
where $44 = C(10,2) - 1$ and $5$ = active D-levels.
...
The exponents are CountQuantity: $C(4,2) = 6$ (\texttt{msMdExponent}), $C(4,2)+C(2,2) = 7$ (\texttt{mtMbExpHigh}), $C(4,2)-C(2,2) = 5$ (\texttt{mtMbExpLow}). All exponents trace to D-structure pair counts, never hardcoded numerals.
