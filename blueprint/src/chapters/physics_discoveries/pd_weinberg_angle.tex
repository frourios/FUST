...
\chapter{Weinberg Angle from \texorpdfstring{$D_\zeta$}{Dζ} Channel Structure}
\label{chap:pd_weinberg}

\section{Kernel Dimension Transition}
\label{sec:weinberg_kernel_transition}

The Weinberg mixing angle is derived from the kernel hierarchy of $D_\zeta$'s component operators.
...
\[
\frac{m_W}{m_Z} = \cos\theta_W = \sqrt{\frac{C(5,2)}{C(3,2)+C(5,2)}} = \sqrt{\frac{10}{13}} \approx 0.877.
\]
...
= \varphi^{25} \times \frac{15}{16} \times \sqrt{\frac{13}{10}}.
\]
The Z boson mass is fully determined by $\varphi$ and pair counts,
with no free parameters.

\textbf{Note on dimensional type}: In Lean, $m_Z^2$ is represented as \texttt{DimSum2 dimZSqComp1 dimZSqComp2},
a formal sum of two ScaleQ with different FDim $(40,-48,-52)$ and $(42,-46,-52)$.
The $\mathbb{R}$-evaluation recovers the physical $m_Z^2 = m_W^2 \times 13/10$.
This dimensional splitting reflects the distinct origins of the two contributions
($m_W^2$ from the $D_6^2$ sector, $m_W^2 \sin^2\theta_W / \cos^2\theta_W$ from the mixed sector).
