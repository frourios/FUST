...
$\alpha = \varphi^{-5}/4\pi$ & ScaleQ$(0,0,0)$ & \texttt{fineStructure} \\
$\theta_C = \arctan(1/\varphi^3)$ & ScaleQ$(0,0,0)$ & \texttt{cabibboAngle} \\
\hline
\end{tabular}
\end{center}

RatioQuantity values are ratios of pair counts $C(m,2)$, ensuring they are dimensionless. ScaleQ$(0,0,0)$ quantities involve transcendental functions ($\pi$, $\arctan$) but remain dimensionless. All definitions in \texttt{FUST\_dim.CouplingConstants} use structural formulas ($C(m,2)$), never hardcoded numerals.
