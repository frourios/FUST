...
\]
This uses $\mathrm{Diff}_3$ (weak sector) vs $\mathrm{Diff}_5$+$\mathrm{Diff}_6$ (unified sector).
\end{theorem}
...
\]
The exponent 3 = $C(3,2)$ from $\mathrm{Diff}_3$ pair count.
\end{theorem}
...
\lean{FUST.CouplingConstants.pmns_solar_from_Diff3_symmetry}
\leanok
\[
\sin^2\theta_{12} = \frac{1}{3}
\]
from $\mathrm{Diff}_3$ three-fold symmetry.
\end{theorem}
...
\]
where 5 = number of active numerator operator levels ($\mathrm{Diff}_2$ through $\mathrm{Diff}_6$).
\end{theorem}
...
\end{align}
The exponent 3 = $C(3,2)$ comes from $\mathrm{Diff}_3$ pair count.
\end{theorem}
...
share the \emph{same denominator}: the strong coupling constant is the ratio of $\mathrm{Diff}_3$ pairs to the
...
\item $\mathrm{Diff}_5$ extended kernel: $\mathrm{Diff}_5[1] = \mathrm{Diff}_5[x] = 0$
\end{enumerate}
\end{theorem}
