...
\chapter{Gravitational Coupling from \texorpdfstring{$D_\zeta$}{Dζ} Structure}
...
\]
where $p_3 = \sum t_k^3$, $e_3 = \prod t_k$, $d = \dim\ker(F_\zeta) = 3$
...
\leanok
\[
5 = 6 - 2 + 1 \text{ (active operator levels)}
\]
\end{theorem}
...
\section{Operator Hierarchy Pair Counts}
\label{sec:grav_pair_counts}

\begin{theorem}[Total Operator Hierarchy Pairs]
...
\item Fractional correction: $5/63 = 5/(C(3,2) \times C(7,2))$
\item Numerator operator pairs: $1 + 3 + 6 + 10 + 15 = 35$
\item $\mathrm{Diff}_3$ gauge invariance: $\mathrm{Diff}_3[1] = 0$
\end{enumerate}
\end{theorem}
...
\item Decoupling factor: $45 = C(3,2) \times C(6,2)$
\item Sector trace squares: $26 = t_1^2 + t_2^2 + t_3^2$
\end{enumerate}
\end{theorem}

\section{Inverse Square Law from $\mathrm{Diff}_6$ Structure}
\label{sec:grav_inverse_square}

Newton's inverse square law $F \propto 1/r^2$ is derived purely from the $\mathrm{Diff}_6$
...
\subsection{Extended Kernel of $\mathrm{Diff}_6$ Structure}

Define the dissipation coefficient for the monomial $t^n$:
\[
C(n) = \varphi^{3n} - 3\varphi^{2n} + \varphi^n - \psi^n + 3\psi^{2n} - \psi^{3n}.
\]
The kernel $\ker(F_\zeta) = \operatorname{span}\{1, t, t^2\}$ corresponds to $C(0) = C(1) = C(2) = 0$.
...
\begin{theorem}[$\mathrm{Diff}_6$ Annihilates $t^{-2}$]
\label{thm:N6_inv_sq_zero}
\lean{FUST.GravitationalCoupling.Diff6_inv_sq_zero}
\leanok
For all $x \neq 0$,
\[
\mathrm{Diff}_6[t^{-2}](x) = 0.
\]
\end{theorem}

\begin{proof}
Expand $\mathrm{Diff}_6[t^{-2}](x)$ using $(ax)^{-2} = a^{-2}x^{-2}$.
...
The $\mathrm{Diff}_6$ operator applied to the $1/r$ potential $\Phi(t) = t^{-1}$ yields:

\begin{theorem}[Inverse-Square Force from $\mathrm{Diff}_6$]
\label{thm:N6_inv_one}
\lean{FUST.GravitationalCoupling.Diff6_inv_one}
\leanok
For all $x \neq 0$,
\[
\mathrm{Diff}_6[t^{-1}](x) = \frac{6}{(\varphi - \psi)^4 \cdot x^2} = \frac{6}{25} \cdot \frac{1}{x^2}.
\]
\end{theorem}
...
For all $x \neq 0$,
\[
\Box_\varphi[t^{-1}](x) = \mathrm{Diff}_6(\mathrm{Diff}_6[t^{-1}])(x) = 0.
\]
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:N6_inv_one}, $\mathrm{Diff}_6[t^{-1}](y) = \tfrac{6}{(\varphi-\psi)^4} \cdot y^{-2}$
for $y \neq 0$.  Since $\mathrm{Diff}_6$ evaluates only at nonzero points $\varphi^k x$ and $\psi^k x$,
the outer $\mathrm{Diff}_6$ sees a constant multiple of $t^{-2}$.
By $\mathrm{Diff}_6$-linearity (homogeneity) and Theorem~\ref{thm:N6_inv_sq_zero}:
\[
\mathrm{Diff}_6\!\Bigl[\frac{6}{(\varphi-\psi)^4} \cdot t^{-2}\Bigr](x)
= \frac{6}{(\varphi-\psi)^4} \cdot \mathrm{Diff}_6[t^{-2}](x) = 0. \qedhere
\]
\end{proof}
...
\leanok
From the $\mathrm{Diff}_6$ structure alone:
\begin{enumerate}
\item $\dim\ker(F_\zeta) = 3$ determines spatial dimension $d = 3$.
\item $\mathrm{Diff}_6[t^{-2}] = 0$: the inverse-square monomial is in the extended kernel.
\item $\mathrm{Diff}_6[t^{-1}](x) = \tfrac{6}{25} x^{-2}$: force is inverse-square with nonzero coefficient.
...
C(-2) = 0
\;\;\Longrightarrow\;\;
\mathrm{Diff}_6[t^{-2}] = 0
\;\;\Longrightarrow\;\;
\Box_\varphi[t^{-1}] = 0
\;\;\Longrightarrow\;\;
F = \mathrm{Diff}_6[t^{-1}] \propto \frac{1}{r^2}.
\]

\section{$\mathrm{Diff}_6$ Structure Algebraic Relations}
\label{sec:grav_d_structure_relations}

Systematic exploration of $\mathrm{Diff}_6$ algebraic properties reveals deep connections
to Fibonacci number theory and the sector factorization.

\subsection{$\mathrm{Diff}_6$ Coefficient Structure}

\begin{theorem}[$\mathrm{Diff}_6$ Evaluation Point Sum]
\label{thm:N6_eval_sum}
\lean{FUST.GravitationalCoupling.Diff6_eval_multiplier_sum}
\leanok
\[
...
\begin{theorem}[$\mathrm{Diff}_6$ Coefficient Properties]
\label{thm:N6_coeff_props}
\lean{FUST.GravitationalCoupling.Diff6_coeff_sum}
\lean{FUST.GravitationalCoupling.Diff6_coeff_abs_sum}
\leanok
The $\mathrm{Diff}_6$ coefficients $[1, -3, 1, -1, 3, -1]$ satisfy:
\begin{enumerate}
...
\subsection{$\mathrm{Diff}_6$ Spectral Invariants}

The elementary symmetric polynomials $e_k$ of the evaluation multipliers
$\{\varphi^3, \varphi^2, \varphi, \psi, \psi^2, \psi^3\}$ are:

\begin{theorem}[$\mathrm{Diff}_6$ Spectral Invariants]
\label{thm:N6_spectral_invariants}
\lean{FUST.GravitationalCoupling.Diff6_spectral_invariants}
\leanok
\begin{align*}
e_1 &= 8, & e_2 &= 18, & e_3 &= 6, \\
...
\]
whose roots are precisely the $\mathrm{Diff}_6$ evaluation multipliers.

\begin{theorem}[$\mathrm{Diff}_6$ Characteristic Polynomial Roots]
\label{thm:N6_charPoly_roots}
\lean{FUST.GravitationalCoupling.Diff6_charPoly_roots}
\leanok
\[
...
\subsection{\texorpdfstring{$\mathrm{Diff}_6$}{Diff₆} Sector Factorization}

The characteristic polynomial factors into three sectors:

\begin{theorem}[$\mathrm{Diff}_6$ Characteristic Polynomial Factorization]
\label{thm:N6_charPoly_factorization}
\lean{FUST.GravitationalCoupling.Diff6_charPoly_factorization}
\leanok
\[
...
\]
The spatial normalization $C(6,3)$ factors as the product of spacetime dimension and active operator levels.
...
The $\mathrm{Diff}_6$ characteristic polynomial factors into three sectors with
...
and the vacuum dimension $d = \dim\ker(F_\zeta) = 3$.
...
\leanok
\[
e_3 + d = t_1 \cdot t_2 \cdot t_3 + \dim\ker(F_\zeta) = 12 + 3 = 15 = C(6,2).
\]
The sector product plus vacuum dimension equals the $\mathrm{Diff}_6$ pair count.
\end{theorem}
...
\leanok
\[
107 = p_3 + e_3 + d = \sum t_k^3 + \prod t_k + \dim\ker(F_\zeta) = 92 + 12 + 3.
\]
...
\label{thm:Fzeta_power_sum_2}
\lean{FUST.NavierStokes.Fζ_power_sum_2}
\leanok
\[
...
The $\mathrm{Diff}_6$ operator maps $\mathrm{Diff}_6[t^n](x) = C(n) \cdot x^n / (\varphi-\psi)^5$
...
\end{itemize}
Since $\mathrm{Diff}_6$ annihilates $\Delta = 1$, mass ratios $m_e/m_{Pl}$
are \emph{boundary data} of the operator hierarchy, not determined by $\mathrm{Diff}_6$ eigenvalues.

\begin{theorem}[$\mathrm{Diff}_6$ Kernel Dimensions]
\label{thm:N6_kernel_dimensions}
\lean{FUST.GravitationalCoupling.Diff6_kernel_dimensions}
\leanok
$\mathrm{Diff}_6[1] = \mathrm{Diff}_6[t] = \mathrm{Diff}_6[t^2] = 0$ (kernel: $\Delta = 0, 1, 2$),
while $\mathrm{Diff}_6[t^{-1}] \neq 0$ (force operator: outside kernel).
\end{theorem}

The physical exponents form a three-layer structure:

\subsection*{Layer 1: $\mathrm{Diff}_6$ Eigenvalue Structure (structural)}

Determined purely by $\mathrm{Diff}_6$ spectral data:
\begin{itemize}
\item $d = \dim(\ker F_\zeta) = 3$ (spatial dimension)
...
of the $\mathrm{Diff}_6$ characteristic polynomial factorization
$p(x) = (x^2{-}x{-}1)(x^2{-}3x{+}1)(x^2{-}4x{-}1)$:
\begin{itemize}
\item $107 = p_3 + e_3 + d = \sum t_k^3 + \prod t_k + \dim\ker(F_\zeta)$
...
from the $\mathrm{Diff}_6$ sector factorization, avoiding the degree-inhomogeneity of earlier spectral formulas.

\section{Graviton Structural Prediction}
\label{sec:grav_graviton}

The graviton is not postulated but derived from the $\mathrm{Diff}_6$ structure.
...
\Box_\varphi[t^{-1}](x) = \mathrm{Diff}_6(\mathrm{Diff}_6[t^{-1}])(x) = 0.
\]
The graviton propagator has no mass term because $\mathrm{Diff}_6[t^{-1}] \propto t^{-2}$
and $t^{-2}$ is in the extended kernel of $\mathrm{Diff}_6$ (Theorem~\ref{thm:N6_inv_sq_zero}).
\end{theorem}
...
(Weinberg's theorem). With $\dim\ker(F_\zeta) = 3$, the graviton is uniquely spin-2.
\end{theorem}

\begin{theorem}[Graviton Prediction from $\mathrm{Diff}_6$ Structure]
\label{thm:graviton_prediction}
\lean{FUST.GravitationalCoupling.graviton_prediction}
\leanok
From the $\mathrm{Diff}_6$ characteristic polynomial and kernel alone:
\begin{enumerate}
...
\item \textbf{Inverse square}: $\mathrm{Diff}_6[t^{-1}](x) = \tfrac{6}{25} x^{-2}$.
\end{enumerate}
\end{theorem}

The derivation chain is:
\[
\mathrm{Diff}_6 \text{ charPoly}
\;\Longrightarrow\;
