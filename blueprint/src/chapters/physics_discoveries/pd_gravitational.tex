...
\section{Inverse Square Law from $N_6$ Structure}
\label{sec:grav_inverse_square}

Newton's inverse square law $F \propto 1/r^2$ is derived purely from the $N_6$
...
\subsection{Extended Kernel of $N_6$ Structure}
...
\begin{theorem}[$N_6$ Annihilates $t^{-2}$]
\label{thm:N6_inv_sq_zero}
\lean{FUST.GravitationalCoupling.N6_inv_sq_zero}
\leanok
For all $x \neq 0$,
\[
N_6[t^{-2}](x) = 0.
...
\begin{proof}
Expand $N_6[t^{-2}](x)$ using $(ax)^{-2} = a^{-2}x^{-2}$.
...
The $N_6$ operator applied to the $1/r$ potential $\Phi(t) = t^{-1}$ yields:

\begin{theorem}[Inverse-Square Force from $N_6$]
\label{thm:N6_inv_one}
\lean{FUST.GravitationalCoupling.N6_inv_one}
\leanok
For all $x \neq 0$,
\[
N_6[t^{-1}](x) = \frac{6}{(\varphi - \psi)^4 \cdot x^2} = \frac{6}{25} \cdot \frac{1}{x^2}.
\]
\end{theorem}
...
For all $x \neq 0$,
\[
\Box_\varphi[t^{-1}](x) = N_6(N_6[t^{-1}])(x) = 0.
\]
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:N6_inv_one}, $N_6[t^{-1}](y) = \tfrac{6}{(\varphi-\psi)^4} \cdot y^{-2}$
for $y \neq 0$.  Since $N_6$ evaluates only at nonzero points $\varphi^k x$ and $\psi^k x$,
the outer $N_6$ sees a constant multiple of $t^{-2}$.
By $N_6$-linearity (homogeneity) and Theorem~\ref{thm:N6_inv_sq_zero}:
\[
N_6\!\Bigl[\frac{6}{(\varphi-\psi)^4} \cdot t^{-2}\Bigr](x)
= \frac{6}{(\varphi-\psi)^4} \cdot N_6[t^{-2}](x) = 0. \qedhere
\]
\end{proof}
...
\leanok
From the $N_6$ structure alone:
\begin{enumerate}
\item $\dim\ker(F_\zeta) = 3$ determines spatial dimension $d = 3$.
\item $N_6[t^{-2}] = 0$: the inverse-square monomial is in the extended kernel.
\item $N_6[t^{-1}](x) = \tfrac{6}{25} x^{-2}$: force is inverse-square with nonzero coefficient.
...
\Box_\varphi[t^{-1}] = 0
\;\;\Longrightarrow\;\;
F = N_6[t^{-1}] \propto \frac{1}{r^2}.
\]

\section{$N_6$ Structure Algebraic Relations}
...
\subsection{$N_6$ Coefficient Structure}

\begin{theorem}[$N_6$ Evaluation Point Sum]
\label{thm:N6_eval_sum}
\lean{FUST.GravitationalCoupling.N6_eval_multiplier_sum}
\leanok
\[
...
\begin{theorem}[$N_6$ Coefficient Properties]
\label{thm:N6_coeff_props}
\lean{FUST.GravitationalCoupling.N6_coeff_sum}
\lean{FUST.GravitationalCoupling.N6_coeff_abs_sum}
\leanok
The $N_6$ coefficients $[1, -3, 1, -1, 3, -1]$ satisfy:
\begin{enumerate}
...
\]
whose roots are precisely the $N_6$ evaluation multipliers.

\begin{theorem}[$N_6$ Characteristic Polynomial Roots]
\label{thm:N6_charPoly_roots}
\lean{FUST.GravitationalCoupling.N6_charPoly_roots}
\leanok
\[
...
\subsection{\texorpdfstring{$N_6$}{N₆} Sector Factorization}

The characteristic polynomial factors into three sectors:

\begin{theorem}[$N_6$ Characteristic Polynomial Factorization]
\label{thm:N6_charPoly_factorization}
\lean{FUST.GravitationalCoupling.N6_charPoly_factorization}
\leanok
\[
...
The $N_6$ characteristic polynomial factors into three sectors with
...
\]
The sector product plus vacuum dimension equals the $N_6$ pair count.
\end{theorem}
...
The $N_6$ operator maps $N_6[t^n](x) = C(n) \cdot x^n / (\varphi-\psi)^5$
...
\end{itemize}
Since $N_6$ annihilates $\Delta = 1$, mass ratios $m_e/m_{Pl}$
are \emph{boundary data} of the operator hierarchy, not determined by $N_6$ eigenvalues.

\begin{theorem}[$N_6$ Kernel Dimensions]
\label{thm:N6_kernel_dimensions}
\lean{FUST.GravitationalCoupling.N6_kernel_dimensions}
\leanok
$N_6[1] = N_6[t] = N_6[t^2] = 0$ (kernel: $\Delta = 0, 1, 2$),
while $N_6[t^{-1}] \neq 0$ (force operator: outside kernel).
\end{theorem}

The physical exponents form a three-layer structure:

\subsection*{Layer 1: $N_6$ Eigenvalue Structure (structural)}

Determined purely by $N_6$ spectral data:
\begin{itemize}
...
of the $N_6$ characteristic polynomial factorization
$p(x) = (x^2{-}x{-}1)(x^2{-}3x{+}1)(x^2{-}4x{-}1)$:
\begin{itemize}
...
from the $N_6$ sector factorization, avoiding the degree-inhomogeneity of earlier spectral formulas.

\section{Graviton Structural Prediction}
\label{sec:grav_graviton}

The graviton is not postulated but derived from the $N_6$ structure.
...
\Box_\varphi[t^{-1}](x) = N_6(N_6[t^{-1}])(x) = 0.
\]
The graviton propagator has no mass term because $N_6[t^{-1}] \propto t^{-2}$
and $t^{-2}$ is in the extended kernel of $N_6$ (Theorem~\ref{thm:N6_inv_sq_zero}).
\end{theorem}
...
\begin{theorem}[Graviton Prediction from $N_6$ Structure]
\label{thm:graviton_prediction}
\lean{FUST.GravitationalCoupling.graviton_prediction}
\leanok
From the $N_6$ characteristic polynomial and kernel alone:
\begin{enumerate}
...
\item \textbf{Inverse square}: $N_6[t^{-1}](x) = \tfrac{6}{25} x^{-2}$.
\end{enumerate}
\end{theorem}
