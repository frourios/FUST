...
\chapter{Gravitational Collapse from D-Structure}
\label{chap:pd_black_hole}

All results derived from $D_6$ operator algebra and $\varphi$-$\psi$ duality only.
...
From $\Delta = 1/t_{\mathrm{FUST}}$ and mass $m = \Delta \cdot x_0^2$,
...
\begin{theorem}[Mass--Time Identity]
\label{thm:bh_mass_time_identity}
\lean{FUST.BlackHole.mass_time_identity}
\leanok
For all $x_0 \in \mathbb{R}$:
\[
\Delta \cdot x_0^2 \cdot t_{\mathrm{FUST}} = x_0^2
\]
\end{theorem}
