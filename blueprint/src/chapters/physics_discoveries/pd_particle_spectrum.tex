...
\leanok
Allowed charge count from $N_3$ structure:
\[
2 \times C(3,2) + 1 = 7
\]
...
\leanok
Colored lepton (hypercharge $Y = -1$, color = triplet) is forbidden by $N_3$-$N_4$ embedding.
\end{theorem}
...
\subsection{Neutrino Mass Structure at \texorpdfstring{$N_5$}{N5}}
...
Right-handed neutrinos predicted at $N_5$ level (spin-1/2).
\end{theorem}

\subsection{\texorpdfstring{$\Phi_S$}{Φ_S} Dark Matter}

$\Phi_S$ is the SY channel operator between $N_5$ and $F_\zeta$:
\[
\Phi_S f(x) = N_5 f(x) + \mu \cdot (f(\varphi x) - f(\psi x)), \quad \mu = \frac{2}{\varphi + 2}.
\]

Kernel hierarchy:
$\ker(\Phi_S) \subset \ker(N_5) \subset \ker(F_\zeta)$, \quad $\dim: 1 \to 2 \to 3$.
\begin{itemize}
\item $\ker(\Phi_S) = \{1\}$: only constants (same as $N_3$)
\item $\Phi_S[x] \neq 0$: detects linear functions (unlike $N_5$ and $F_\zeta$)
\item Gauge-invariant output: $\Phi_S[x](x)/x = 2/\varphi \approx 1.236$
\end{itemize}

$\mathrm{deriveFDim}(\Phi_S) = (-4, 0, 1)$:
same denominator structure as $N_5$ ($p = -4$), antisymmetric ($\delta = 0$),
but $\ker\dim = 1$ gives $\tau = 2 - 1 = 1$ (vs.\ $N_5$: $\tau = 0$, $F_\zeta$: $\tau = -1$).
...
\item Layer 2 ($\ker(N_5) \setminus \ker(\Phi_S)$): $\Phi_S$ detects $x$;
  photon/gluon transitions
\item Layer 3 ($\ker(F_\zeta) \setminus \ker(N_5)$): $N_5$ detects $x^2$;
...
\item \textbf{Mixed sector}: $Z^2$ comp2.
  Involves $N_2$ and $N_3$ corrections.
...
The $N_4$ anomaly ($N_4[1] \neq 0$ but $N_4[x^2] = 0$) explains why the
$\tau/\mu$ exponent equals $C(4,2) = 6$: the non-contiguous kernel of $N_4$
...
\]
These are the pair counts $C(m,2)$ of the 5 active numerator operators $N_2, \ldots, N_6$.
\end{theorem}
...
\end{align}
\begin{itemize}
\item $e \to \mu$: $C(5,2) + C(2,2) = 11$ from $N_5$ kernel expansion ($\ker\dim: 1 \to 2$)
  plus $N_2$ boundary.
\item $\mu \to \tau$: $C(4,2) = 6$ from $N_4$'s anomalous kernel (the only operator
...
\]
The W boson exponent is the total pair count of $N_5 + N_6$.
\end{theorem}
