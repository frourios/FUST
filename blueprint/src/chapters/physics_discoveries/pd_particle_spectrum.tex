...
\begin{itemize}
\item SY channel: weight 3 (carries SU(3) color, mode space rank 3)
\item AF channel: weight 1 (carries SU(2) weak, quaternionic dim 2)
...
Each $\ker(F_\zeta)$ basis element $x^d$ has $\mathrm{FDim} = \mathrm{dimScale}^d$,
...
\item \textbf{Ratio sector}: pure $\mathrm{dimScale}^n$.
...
This follows from the injectivity of the map $(a, n) \mapsto \mathrm{deriveFDim}(6)^a \times \mathrm{dimScale}^n$.
\end{theorem}
