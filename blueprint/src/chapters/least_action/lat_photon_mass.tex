...
\leanok
Constant functions belong to $\ker(D_6)$.
\end{theorem}
...
\leanok
$\ker(D_6) = \text{Constant functions} \lor \text{Non-constant functions}$
\end{theorem}
...
\begin{tabular}{|c|c|}
\hline
Physical property & Correspondence in $D_6$ structure \\
\hline
Mass $m = 0$ & No components of degree 3 or higher (belongs to $\ker(D_6)$) \\
...
\leanok
$\mathrm{IsPhotonState}\, f \iff \mathrm{IsInKerD6}\, f \land \mathrm{HasEnergy}\, f$
\end{definition}
...
\leanok
\[
\mathrm{IsPhotonState}\, f \iff f \in \ker(D_6) \land (f(1) \neq f(0) \lor f(-1) \neq f(0))
\]
\end{theorem}
...
\textbf{The definition of mass states is not arbitrary, but derived as deviation from $\ker(D_6)$:}
...
\begin{tabular}{|c|c|c|}
\hline
State & $D_6$ structure & Physical meaning \\
\hline
Vacuum & Constant function $\in \ker(D_6)$ & No energy, no mass \\
Photon & Non-constant $\in \ker(D_6)$ & Has energy, no mass, no proper time \\
Matter & $f \notin \ker(D_6)$ & Has energy, has mass, has proper time \\
\hline
\end{tabular}
\end{center}
...
By linearity of $D_6$ and the scaling law:
\[
D_6[f](x) = \sum_{n \geq 3} a_n D_6[t^n](x)
\]
...
\[
\frac{|D_6 f_A(x)|}{|D_6 f_B(x)|} \propto x^{n-m}
\]
