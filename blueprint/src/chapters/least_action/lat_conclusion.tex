...
Item & Content & Status \\
\hline
Action functional & $\mathcal{A}[f] = \int_0^\infty \|\mathrm{Diff}_6 f\|^2 \frac{dx}{x}$ & Derived \\
...
Euler-Lagrange & $\ker(\mathrm{Diff}_6^\dagger \mathrm{Diff}_6) = \ker(F_\zeta)$ & Derived \\
...
\item \textbf{Higher Order Reduction Theorem} (Diff7\_kernel\_equals\_Diff6\_kernel): $\ker(\mathrm{Diff}_7) = \ker(F_\zeta)$
...
\item \textbf{Gauge Transformation Rule} (Diff6\_homogeneous): $\mathrm{Diff}_6[f(c\cdot)](x) = c \cdot \mathrm{Diff}_6[f](cx)$. $\varphi$-scaling is time evolution via Poincar\'{e} $I_4$ temporal channel (Gravity.lean: $\Phi$\_A\_coeff\_one, $\eta$\_temporal, temporal\_nonzero)
...
\item \textbf{Mass Gap Theorem} (yangMills\_massGap): $F_\zeta$ minimum eigenvalue $\lambda_{\min} = 12/25 > 0$; Casimir mass gap $m^2 = 14$ from $D_\zeta$ 4-momentum; SU(2) mass gap via $\mathrm{Diff}_5(x^2) \neq 0$; algebraic confinement from $\ker(F_\zeta)$ not being a subalgebra
