...
\hline
$f \in \ker(D_6)$ & D6\_const, D6\_linear, D6\_quadratic & Non-existence of time (light-like state) \\
...
\begin{enumerate}
\item \textbf{Time--Mass--Lagrangian Unification}: $f \notin \ker(D_6) \iff$ time exists $\iff$ $L[f] > 0$ $\iff$ $m^2 > 0$
...
\item \textbf{Gauge Transformation Rule} (D6\_gauge\_scaling): $D_6[f(c\cdot)](x) = c \cdot D_6[f](cx)$. $\varphi$-scaling is time evolution via Poincar\'{e} $I_4$ temporal channel (Gravity.lean: $\Phi$\_A\_coeff\_one, $\eta$\_temporal, temporal\_nonzero)
...
\item \textbf{Mass Gap Theorem} (yangMills\_massGap): $D_6$ minimum eigenvalue $\lambda_{\min} = 12/25 > 0$; Casimir mass gap $m^2 = 14$ from $D_\zeta$ 4-momentum; SU(2) mass gap via $D_5(x^2) \neq 0$; algebraic confinement from $\ker(F_\zeta)$ not being a subalgebra
