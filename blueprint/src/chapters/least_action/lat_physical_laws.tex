...
The gauge group $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ is uniquely determined by three formally proved uniqueness theorems:
\begin{itemize}
\item \textbf{SU(3)}: SY mode space has rank 3 ($\det M_{\mathrm{SY}} \neq 0$), norm preservation $\to$ $\mathrm{U}(3)$, scalar center separation, $\det = 1$ $\to$ $\mathrm{SU}(3)$ (\texttt{SU3\_gauge\_uniqueness})
\item \textbf{SU(2)}: $\tau(\mathrm{AF\_coeff}) = -\mathrm{AF\_coeff}$ (quaternionic type, $J^2 = -12 < 0$), doubling $\mathbb{C}^1 \to \mathbb{C}^2 = \mathbb{H}^1$ $\to$ $\mathrm{SU}(2) \cong \mathrm{Sp}(1)$ (\texttt{SU2\_gauge\_uniqueness})
\item \textbf{U(1)}: Galois-fixed subspace is 1D ($b = c = d = 0$), $\mathrm{SU}(1) = \{I\}$ trivial $\to$ $\mathrm{U}(1)$ (\texttt{U1\_gauge\_uniqueness})
\end{itemize}
...
Gauge group & $\mathbb{Z}/6\mathbb{Z}$ channel decomposition & \texttt{SU3/SU2/U1\_gauge\_uniqueness} \\
