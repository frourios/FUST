...
\section{Gauge Groups from \texorpdfstring{$F_\zeta$}{Fζ} Channels}
\label{sec:mat_gauge_groups}

The Standard Model gauge group is uniquely determined by $F_\zeta$'s channel decomposition:

\begin{theorem}[Standard Gauge Group Uniqueness]
\label{thm:mat_gauge_unique}
\lean{FUST.standard_gauge_group_unique}
\leanok
$F_\zeta$'s $\mathbb{Z}/6\mathbb{Z}$ channel structure uniquely determines $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$:
\begin{enumerate}
\item $\Phi_S$ rank 3 (SY channel, $\mathrm{syWeight} = 3$) $\Rightarrow$ $\mathrm{SU}(3)$
\item $\mathrm{AF\_coeff} = 2i\sqrt{3}$ purely imaginary (AF channel) $\Rightarrow$ $\mathrm{SU}(2)$
...
\leanok
$\zeta_6 \neq \varphi^n$ for all $n$, ensuring non-reducibility.
\end{theorem}
