...
\begin{quote}
\textbf{To derive time, the action theorem, and causal structure solely from FUST's $F_\zeta$ structure.}
\end{quote}
...
\textbf{Important}: FUST has no ``principles.'' Minimal action is not a principle assumed externally, but \textbf{derived as a theorem} from $F_\zeta$ structure.

\section{Overview of FUST and $F_\zeta$ Structure}
\label{sec:mat_d6_overview}

\subsection{Definition of the N6 Operator}

The $N_6$ operator is defined using the golden ratio $\varphi = (1+\sqrt{5})/2$ and its conjugate $\psi = (1-\sqrt{5})/2$:

\begin{definition}[$N_6$ Operator]
\label{def:mat_D6}
\lean{FUST.N6}
\leanok
\[
N_6[f](x) = \frac{f(\varphi^3 x) - 3f(\varphi^2 x) + f(\varphi x) - f(\psi x) + 3f(\psi^2 x) - f(\psi^3 x)}{(\varphi - \psi)^5 x}
\]
\end{definition}

The coefficients $[1, -3, 1, -1, 3, -1]$ are uniquely determined from golden ratio relations (proven in Lean4). The unified operator $F_\zeta = 5z \cdot D_\zeta$ combines this numerator structure with the $\zeta_6$ arithmetic factor.
...
$N_7$ and higher do not introduce new structure. $F_\zeta$ determines the ``minimal and complete'' causal structure.

\textbf{Numerical verification}: Coefficient $C_n$ for monomials $t^n$ under $N_6$:
...
The parameter $x$ in the definition of $N_6$ is a gauge choice, not a physical quantity.
...
\lean{FUST.GravitationalCoupling.N6_homogeneous}
\leanok
\[
N_6[f(c \cdot)](x) = c \cdot N_6[f](cx)
\]
\end{theorem}
