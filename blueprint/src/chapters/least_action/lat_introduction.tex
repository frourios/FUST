...
\section{Overview of FUST and \texorpdfstring{$F_\zeta$}{Fζ} Structure}
\label{sec:mat_d6_overview}

\subsection{Definition of the Diff6 Operator}

The $\mathrm{Diff}_6$ operator is defined using the golden ratio $\varphi = (1+\sqrt{5})/2$ and its conjugate $\psi = (1-\sqrt{5})/2$:
...
$\mathrm{Diff}_7$ and higher do not introduce new structure. $F_\zeta$ determines the ``minimal and complete'' causal structure.

\textbf{Numerical verification}: Coefficient $C_n$ for monomials $t^n$ under $\mathrm{Diff}_6$:
...
The parameter $x$ in the definition of $\mathrm{Diff}_6$ is a gauge choice, not a physical quantity.
...
\lean{FUST.GravitationalCoupling.Diff6_homogeneous}
\leanok
\[
\mathrm{Diff}_6[f(c \cdot)](x) = c \cdot \mathrm{Diff}_6[f](cx)
\]
\end{theorem}
...
\[
\mathrm{Diff}_6[t^n](x) \propto x^{n-1}
\]
