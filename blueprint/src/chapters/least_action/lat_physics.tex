...
The comparison $\Delta^2 \leq E$ in \texttt{EnergyInSpectrum} compares quantities of the same dimension $(-10, 2, -2)$, which is dimensionally consistent. $\Delta = 12/25$ (ScaleQuantity from $D_6$ gauge-invariant output) has dimension $(-5, 1, -1) = \mathrm{dimElectron}$, the electron baseline. Other particles acquire distinct FDim via $\varphi$-scaling (see Chapter~\ref{chap:pd_mass_ratios}).
