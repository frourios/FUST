...
$\Delta = \dim\ker(D_5) \times \dim\ker(D_6)$ & CountQuantity & $6$ \\
$\Delta^2$ (energy threshold) & CountQuantity & $36$ \\
...
The comparison $\Delta^2 \leq E$ in \texttt{EnergyInSpectrum} is a \textbf{numerical comparison} in the FUST coordinate system, not an equality of dimensioned quantities. $\Delta = 6$ (CountQuantity from kernel tensor product) is structurally distinct from $\varphi^6 \approx 17.94$ (ScaleQuantity, mass ratio).

Lean: \texttt{FUST\_dim.Combined.massGap} (CountQ), \texttt{FUST\_dim.Combined.spectralGap\_count}.
