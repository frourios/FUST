...
\hline
Lagrangian $L = T - V$ & $L_{\mathrm{FUST}} = \|N_6 f\|^2$ \\
...
\item No mass (photon) $\leftrightarrow$ Proper time does not exist ($N_6 f = 0$)
\end{itemize}
...
\hline
$N_5$ & $\dim \ker(N_5) = 2$ & $\mathrm{U}(1)^2$ & $\mathrm{SU}(2)$ \\
$N_6$ & $\dim \ker(F_\zeta) = 3$ & $\mathrm{U}(1)^3$ & $\mathrm{SU}(3)$ \\
\hline
\end{tabular}
\end{center}
...
\[
\dim \ker(N_5) = 2 \;\to\; \mathfrak{su}(2) \;(2^2 - 1 = 3), \qquad
...
The mass gap is derived from the $N_6$ gauge-invariant output:
...
$N_6(t^d)(x) = C_d \cdot x^{d-1} / (\sqrt{5})^5$ where $C_d \cdot x^{d-1} / (\sqrt{5})^5 \cdot x^{-(d-1)} = C_d/(\sqrt{5})^5$ is gauge-invariant (independent of probe point $x$).

Kernel structure:
\begin{itemize}
\item $\ker(F_\zeta) = \mathrm{span}\{1, x, x^2\}$, $\dim = 3$ ($N_6$ annihilates up to degree 2, not degree 3)
...
\[
\forall x \neq 0, N_6[t^2](x) = 0 \land N_6[t^3](x) \neq 0
\]
\end{theorem}
...
\begin{itemize}
\item $\ker(F_\zeta) = \mathrm{span}\{1, x, x^2\}$ is uniquely determined from $N_6$ annihilation structure
\item $C_3 = 12\sqrt{5}$ is the unique value of $N_6$ applied to $x^3$
...
\hline
$L_{\mathrm{FUST}} = \|N_6 f\|^2$ & ScaleQuantity & $(-10, 2, -2)$ \\
