...
Behavior of $N_6$ evaluation points:
\begin{itemize}
...
\[
\dim(S) = \dim((N_6 f)^2) = (-10, 2) \quad (\text{ScaleQuantity})
\]
Time evolution amplifies $N_6 f$ by $\varphi$, so $S' = \varphi^2 \cdot S$ in coordinate time, consistent with the proper-time energy conservation $E_{\mathrm{conserved}} = H / \varphi^{2 \cdot \mathrm{step}}$.
