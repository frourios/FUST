...
The core asymmetry of the $F_\zeta$ structure lies in the absolute values of the golden ratio $\varphi$ and its conjugate $\psi$:
...
Behavior of $\mathrm{Diff}_6$ evaluation points:
\begin{itemize}
...
Photons ($\ker(F_\zeta)$) are invariant under time evolution:
...
Quantify the deviation from $\ker(F_\zeta)$ as entropy:
...
Thus, time evolution amplifies $\ker(F_\zeta)^\perp$ components ($n \geq 3$) by a factor of $\varphi^n$.

\section{Summary of Arrow of Time}
\label{sec:mat_arrow_summary}

The following are derived from $F_\zeta$ and Poincar\'{e} $I_4$ structure:
\begin{enumerate}
...
\textbf{Important}: These are derived from $F_\zeta$ and $D_\zeta$ structure (including the Poincar\'{e} $I_4$ temporal channel identification), without external assumptions (initial conditions, statistical assumptions, external time parameter).
...
\[
\dim(S) = \dim((\mathrm{Diff}_6 f)^2) = (-10, 2) \quad (\text{ScaleQuantity})
\]
Time evolution amplifies $\mathrm{Diff}_6 f$ by $\varphi$, so $S' = \varphi^2 \cdot S$ in coordinate time, consistent with the proper-time energy conservation $E_{\mathrm{conserved}} = H / \varphi^{2 \cdot \mathrm{step}}$.
