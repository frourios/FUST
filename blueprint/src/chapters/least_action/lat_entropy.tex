...
\lean{FUST.monomial_amplification}
\leanok
\[
...
\begin{theorem}[Arrow of Time Summary]
\label{thm:mat_arrow_summary}
\lean{FUST.arrow_of_time_summary}
\leanok
The following are derived from $D_6$ structure:
\begin{enumerate}
\item $\varphi > 1$ (definition of future direction)
\item $|\psi| < 1$ (definition of past direction)
\item $\ker(D_6)$ is invariant under time evolution (time invariance of photons)
\item $D_6[\mathrm{timeEvolution}(f)](x) = \varphi \cdot D_6[f](\varphi x)$ (gauge transformation rule)
\end{enumerate}
\end{theorem}
...
\[
\dim(S) = \dim((D_6 f)^2) = (-10, 2, -2) \quad (\text{ScaleQuantity})
\]
Time evolution amplifies $D_6 f$ by $\varphi$, so $S' = \varphi^2 \cdot S$ in coordinate time. The $\tau = -2$ component indicates inverse-square-time scaling, consistent with the proper-time energy conservation $E_{\mathrm{conserved}} = H / \varphi^{2 \cdot \mathrm{step}}$.
