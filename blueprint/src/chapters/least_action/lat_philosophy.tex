...
FUST is a \textbf{theorem derived} from the ZF axiom system:
\[
\text{ZF} \vdash \text{``The gauge group is } \mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)\text{''}
\]
...
\begin{enumerate}
\item FUST is a theorem derived from ZF
\item ``Contradicting FUST'' means ``contradicting a theorem of ZF''
\item Phenomena contradicting ZF theorems are \textbf{mathematically indescribable}
...
Kant & FUST \\
\hline
Spacetime is an a priori form of intuition & Spacetime structure derived from $D_6$ \\
Causality is an a priori category & Physical laws derived from ZF \\
``Thing-in-itself'' is unknowable & ``World outside ZF'' is outside science \\
\hline
\end{tabular}
\end{center}

FUST \textbf{proves} that ``physical laws are derived a priori (from ZF).''
...
The mathematical structure corresponding to the observable universe is \textbf{unique} as the minimal element of $D_6$.
\end{theorem}

\begin{theorem}[Indefinability of Multiverse]
\label{thm:mat_multiverse}
The concept of ``multiple universes'' is \textbf{undefinable} under ZF + FUST.
\end{theorem}

\begin{proof}
\begin{enumerate}
\item In FUST, physical laws are uniquely derived from ZF
\item ``A universe with different physical laws'' contradicts ZF theorems
\item What contradicts ZF theorems is undefinable within ZF
...
Free thoughts and internal states without causality belong to $\ker(D_6)$. However, to manifest as physical phenomena, projection onto $\ker(D_6)^\perp$ is necessary.
