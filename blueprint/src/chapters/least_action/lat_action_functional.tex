...
\leanok
\[
\boxed{\mathcal{A}[f] = \int_0^\infty |\mathrm{Diff}_6 f(x)|^2 \frac{dx}{x}}
\]
\end{definition}
...
For the action functional $\mathcal{A}[f] = \int |\mathrm{Diff}_6 f|^2 d\mu$, the integral variation gives:
\[
\delta\mathcal{A} = 2 \int \mathrm{Diff}_6 f \cdot \mathrm{Diff}_6(\delta f) \, d\mu = 2 \int \delta f \cdot \mathrm{Diff}_6^\dagger(\mathrm{Diff}_6 f) \, d\mu
\]

\begin{theorem}[Euler-Lagrange Equation]
\label{thm:mat_euler_lagrange}
\[
\mathrm{Diff}_6^\dagger(\mathrm{Diff}_6 f) = 0
\]
\end{theorem}

Since $\mathrm{Diff}_6^\dagger \mathrm{Diff}_6$ is self-adjoint:
\[
\ker(\mathrm{Diff}_6^\dagger \mathrm{Diff}_6) = \ker(F_\zeta)
\]
