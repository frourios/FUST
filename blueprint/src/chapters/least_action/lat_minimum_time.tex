...
The structural minimum length has $\dim(l_{\mathrm{FUST}}) = (5, -1, 0)$: same $(\sqrt{5}, \delta)$ but $\tau = 0$ (spatial). The numerical equality $t_{\mathrm{FUST}} = l_{\mathrm{FUST}} = 25/12$ reflects identical $(\sqrt{5}, \delta)$ components; the $\tau$ distinction is preserved by the type system.

Lean: \texttt{FUST\_dim.TimeTheorem.structuralMinTime\_dim}, \texttt{structuralMinLength\_dim}.
