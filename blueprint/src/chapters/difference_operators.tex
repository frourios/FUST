...
Each $D_m$ operator has a structurally determined output dimension $(p, \delta, \tau)$ (see Chapter~\ref{chap:dimensional_analysis}):

\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
Operator & Coefficient symmetry & Output dimension $(p, \delta, \tau)$ & Lean \\
\hline
...
The $p$ component $-(m-1)$ comes from the normalization denominator $(\varphi-\psi)^{m-1}$.
...
Since all five operators have distinct $(p, \delta, \tau)$, addition $D_i f(x) + D_j f(x)$ ($i \neq j$) is a type error.
