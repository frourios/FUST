...
\section{Component Operators \texorpdfstring{$N_n$}{Nn} (ZF + DC)}
...
\begin{definition}[$N_2$: 2-Point Difference]
\label{def:D2}
\lean{FUST.N2}
\leanok
\[
N_2 f(z) = \frac{f(\varphi z)-f(\psi z)}{(\varphi-\psi)z}
\]
\end{definition}

\begin{definition}[$N_3$: 3-Point Difference]
\label{def:D3}
\[
N_3 f(z) = \frac{f(\varphi^2 z) - f(\varphi z) + f(\psi z)}{(\varphi-\psi)^2 z}
\]
\end{definition}

\begin{definition}[$N_4$: 4-Point Difference]
\label{def:D4}
\[
N_4 f(z) = \frac{f(\varphi^2 z) - \varphi^2 f(\varphi z) + \psi^2 f(\psi z) - f(\psi^2 z)}{(\varphi-\psi)^3 z}
\]
\end{definition}

\begin{definition}[$N_5$: 5-Point Difference]
\label{def:D5}
\lean{FUST.N5}
\leanok
\[
N_5 f(z) = \frac{f(\varphi^2 z) + f(\varphi z) - 4f(z) + f(\psi z) + f(\psi^2 z)}{(\varphi-\psi)^4 z}
\]
\end{definition}

\begin{definition}[$N_6$: 6-Point Difference]
\label{def:D6}
\lean{FUST.N6}
\leanok
\[
...
The elliptic kernel has the same dimension 3 as the hyperbolic $\ker(N_6)$.
\end{theorem}
...
The component operators $N_2, \ldots, N_6$ are recovered as specific projections of $D_\zeta$:
...
\begin{theorem}[$N_5$ and $N_6$ Coefficient Uniqueness]
\label{thm:coeff_uniqueness}
\lean{FUST.N5_coefficients_unique, FUST.N6_coefficients_unique}
\leanok
The only coefficients satisfying the kernel conditions are $a=-1, b=-4$ ($N_5$) and $A=3, B=1$ ($N_6$).
\end{theorem}

Since all five operators have distinct dimensions, $N_i f(z) + N_j f(z)$ ($i \neq j$) is a type error.
