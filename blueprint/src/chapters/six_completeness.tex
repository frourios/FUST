...
The $D_n$ operators are defined for \textbf{arbitrary functions} $f : \mathbb{C} \to \mathbb{C}$.
...
\leanok
\[
\ker(D_n)|_{\mathbb{C}[x]} = \ker(D_6)|_{\mathbb{C}[x]} = \mathrm{span}\{1, x, x^2\} \quad \text{for } n \ge 7
\]
\end{theorem}
...
\textbf{Remark}: The polynomial kernel $\ker(D_6) \cap \mathbb{C}[x] = \operatorname{span}\{1, x, x^2\}$ (dimension 3) is the physically relevant kernel used for spatial dimension derivation.
