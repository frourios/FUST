...
## Two-Level Constraint Structure

**Algebraic constraint**: The golden quadratic field Q(φ) = {a + bφ | a, b ∈ ℚ}
is the unique minimal Frourio-admissible subfield of ℝ. PBW faithfulness requires
the coefficient field Q(φ).

**Physical constraint**: Physically manifest initial values (generated by finite
compositions of Frourio operations) belong to the golden integer ring
ℤ[φ] = {a + bφ | a, b ∈ ℤ}. No denominators arise from finite compositions.
...
/-!
## Golden Integer Ring ℤ[φ] and Golden Quadratic Field Q(φ)
-/

/-- Membership in ℤ[φ] = {a + bφ | a, b ∈ ℤ} -/
def InGoldenInt (x : ℝ) : Prop :=
  ∃ a b : ℤ, x = (a : ℝ) + (b : ℝ) * φ

/-- ℤ[φ] as a subset of ℝ -/
def goldenIntSet : Set ℝ := { x | InGoldenInt x }
...
/-- ℤ[φ] ⊂ Q(φ) as sets -/
theorem goldenIntSet_subset_goldenFieldSet : goldenIntSet ⊆ goldenFieldSet := by
  intro x ⟨a, b, hx⟩
  exact ⟨a, b, by simp [hx]⟩

/-- GoldenInt element belongs to ℤ[φ] -/
theorem goldenInt_in_goldenInt (g : GoldenInt) : InGoldenInt g.toReal := by
  refine ⟨g.a, g.b, ?_⟩
...
/-!
## Physical Initial Value Constraint: ℤ[φ]

While PBW faithfulness requires the coefficient field Q(φ), physical generation
processes (finite compositions of Frourio operations) produce only ℤ[φ] coefficients.
The difference S_{n-1} = φ^{n-1} - φ^{-(n-1)} and scale factors φⁿ are all in ℤ[φ].
No denominators arise from finite compositions.
-/

/-- Physically manifest initial value (generated by finite Frourio operations) -/
def IsPhysicallyManifest (x₀ : ℝ) : Prop := InGoldenInt x₀

/-- φⁿ ∈ ℤ[φ] for all n ∈ ℤ (using phiPow from GoldenValuation) -/
theorem goldenInt_phi_zpow (n : ℤ) : InGoldenInt (φ ^ n) := by
  have h := phiPow_toReal n
  let g := GoldenInt.phiPow n
  refine ⟨g.a, g.b, ?_⟩
  simp only [GoldenInt.toReal] at h
  rw [← h]

/-- S_{n-1} = φ^{n-1} - φ^{-(n-1)} ∈ ℤ[φ] -/
theorem goldenInt_S_coeff (n : ℤ) : InGoldenInt (φ ^ n - φ ^ (-n)) := by
  obtain ⟨a₁, b₁, h₁⟩ := goldenInt_phi_zpow n
  obtain ⟨a₂, b₂, h₂⟩ := goldenInt_phi_zpow (-n)
  refine ⟨a₁ - a₂, b₁ - b₂, ?_⟩
  rw [h₁, h₂]; push_cast; ring

/-- Physical initial value theorem: physically manifest x₀ belongs to ℤ[φ] -/
theorem initial_value_in_goldenInt (x₀ : ℝ)
    (h : IsPhysicallyManifest x₀) : InGoldenInt x₀ := h

/-- Converse: any x₀ ∈ ℤ[φ] is physically manifest -/
theorem goldenInt_is_physically_manifest (x₀ : ℝ) (hx₀ : InGoldenInt x₀) :
    IsPhysicallyManifest x₀ := hx₀

/-- **Physical Equivalence**: x₀ is physically manifest iff x₀ ∈ ℤ[φ] -/
theorem physically_manifest_iff_goldenInt (x₀ : ℝ) :
    IsPhysicallyManifest x₀ ↔ InGoldenInt x₀ := Iff.rfl

/-- φ-orbit of physical initial value lies entirely in ℤ[φ] -/
theorem compatible_orbit_in_goldenInt {x₀ : ℝ} (hx₀ : InGoldenInt x₀) :
    ∀ y ∈ phiOrbit x₀, InGoldenInt y := by
  intro y hy
  obtain ⟨n, hn⟩ := hy
  rw [hn]
  obtain ⟨a₀, b₀, h₀⟩ := hx₀
  obtain ⟨aₙ, bₙ, hₙ⟩ := goldenInt_phi_zpow n
  rw [h₀, hₙ]
  have hφ2 : φ * φ = φ + 1 := by have := golden_ratio_property; rwa [sq] at this
  refine ⟨aₙ * a₀ + bₙ * b₀, aₙ * b₀ + bₙ * a₀ + bₙ * b₀, ?_⟩
  push_cast
  have : (↑bₙ : ℝ) * φ * (↑b₀ * φ) = ↑bₙ * ↑b₀ * (φ + 1) := by rw [← hφ2]; ring
  linarith [this]
