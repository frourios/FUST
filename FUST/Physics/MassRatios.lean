...
/-! ## Mass Ratio Exponents

Mass ratios are φ^n where n comes from D-structure pair counts. -/

def msMdExponent : ℕ := Nat.choose 4 2
def mcMsValue : ℕ :=
  FUST.QuarkMassRatios.mc_ms_component +
   FUST.QuarkMassRatios.mc_ms_isospin_component
def mbMcValue : ℕ := Nat.choose 3 2
def mtMbExpHigh : ℕ := FUST.QuarkMassRatios.mt_mb_exp_high
def mtMbExpLow : ℕ := FUST.QuarkMassRatios.mt_mb_exp_low

theorem msMdExponent_val : msMdExponent = 6 := rfl
theorem mcMsValue_val : mcMsValue = 12 := rfl
theorem mbMcValue_val : mbMcValue = 3 := rfl
theorem mtMbExpHigh_val : mtMbExpHigh = 7 := rfl
theorem mtMbExpLow_val : mtMbExpLow = 5 := rfl
...
theorem muMdRatio_val : muMdRatio = 1 / 2 := by
  simp only [muMdRatio, Nat.choose]; norm_num

/-! ## Mass Ratio Correction Exponents -/

def tauMuExponent : ℕ := Nat.choose 4 2
def muEExponent : ℕ := Nat.choose 5 2 + Nat.choose 2 2

theorem tauMuExponent_val : tauMuExponent = 6 := rfl
theorem muEExponent_val : muEExponent = 11 := rfl

/-! ## Dark Matter Exponent -/

def darkMatterExponent : ℕ := FUST.MassRatioPredictions.darkMatterExponent

theorem darkMatterExponent_val : darkMatterExponent = 25 := rfl

theorem darkMatterExponent_from_pairs :
    darkMatterExponent = Nat.choose 5 2 + Nat.choose 6 2 := rfl

/-! ## Baryon Asymmetry Exponent -/

def baryonExponent : ℕ := FUST.MassRatioPredictions.baryonExponent

theorem baryonExponent_val : baryonExponent = 44 := rfl
...
theorem exponent_sum : tauMuExponent + muEExponent = 17 := rfl

theorem all_exponents_from_pair_counts :
    (msMdExponent = Nat.choose 4 2) ∧
    (mbMcValue = Nat.choose 3 2) ∧
    (mtMbExpHigh = Nat.choose 4 2 + Nat.choose 2 2) ∧
    (mtMbExpLow = Nat.choose 4 2 - Nat.choose 2 2) :=
  ⟨rfl, rfl, rfl, rfl⟩

/-! ## Particle FDim: each particle has a unique dimension

FDim = deriveFDim(6)^a × dimScale^n: the sector exponent a and φ-power n
determine the physical properties. φ^n scaling changes the dimension. -/

def dimElectron : FDim := dimSector
def dimMuon : FDim := dimSector * dimScale ^ (11 : ℤ)
def dimTau : FDim := dimSector * dimScale ^ (17 : ℤ)
def dimProton : FDim := dimSector * dimScale ^ (14 : ℤ)
...
-- Higgs components
def dimHiggsVacuum : FDim := dimWBoson * dimScale
def dimHiggsCorrection : FDim := dimWBoson * (dimScale * dimScale)⁻¹
